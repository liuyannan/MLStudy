...
class LeNet(object):
    ...
    def FGS_train(self,guard_dist=0.25):
        ...
        while (epoch < self.n_epochs) and (not done_looping):
            ...
            for minibatch_index in range(self.n_train_batches):
                ...
                guard_x = ori_x - guard_dist * numpy.sign(self.grad_predytox_func(doublex, doubley))
...
if __name__ == '__main__':
    ...
    params = nn.FGS_train(guard_dist=0.15)
    f = open('./weight/normal_weights_RandInit_FGS_0.15.pkl', 'wb')
    pickle.dump(params, f, protocol=pickle.HIGHEST_PROTOCOL)
    f.close()
    nn = LeNet(train_divisor=2, n_epochs=40)
    params = nn.FGS_train(guard_dist=0.05)
    f = open('./weight/normal_weights_RandInit_FGS_0.05.pkl', 'wb')
    pickle.dump(params, f, protocol=pickle.HIGHEST_PROTOCOL)
    f.close()
