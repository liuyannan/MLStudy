...
class FCSoftMaxLayer(object):
    ...
    def __init__(self, input, n_in, n_out, rng=None, temperature= 1):
        ...
        self.p_y_given_x_notemperature = T.nnet.gg(T.dot(input, self.W) + self.b)
        self.s_y_given_x = T.dot(input, self.W) + self.b

        # symbolic description of how to compute prediction as class whose
        # probability is maximal
        self.y_pred = T.argmax(self.p_y_given_x_notemperature, axis=1)
        # end-snippet-1
    ...
    def confidence_mean(self, y):
        return T.mean(self.p_y_given_x_notemperature[T.arange(y.shape[0]), y])
...
def load_data(dataset,softlabel):
    ''' Loads the dataset
    ...
    with open(softlabel,'rb') as f:
        train_sy, valid_sy = pickle.load(f)
    ...
    train_set_sy = theano.shared(numpy.asarray(train_sy, dtype=theano.config.floatX), borrow=True)
    valid_set_sy = theano.shared(numpy.asarray(valid_sy, dtype=theano.config.floatX), borrow=True)

    rval = [(train_set_x, train_set_y), (valid_set_x, valid_set_y),
            (test_set_x, test_set_y),train_set_sy,valid_set_sy]
    return rval



class LeNet(object):
    def __init__(self, softlabel, mu=0.5, learning_rate=0.1, n_epochs=40, dataset='mnist.pkl.gz', nkerns=[20, 50],
                 batch_size=500, lam_contractive=0, lam_l2=0.01,temperature=1):
        ...
        self.datasets = load_data(dataset,softlabel)
        ...
        self.train_set_sy = self.datasets[3]
        self.valid_set_sy = self.datasets[4]
        ...
        self.layer0 = LeNetConvPoolLayer(self.rng, input=layer0_input, image_shape=(self.train_batch_size, 1, 28, 28),
        ...
        self.layer3 = FCSoftMaxLayer(input=layer3_input, n_in=500, n_out=10,rng=self.rng,temperature=temperature)
        ...
        #                      ,sequences=[T.arange(self.train_batch_size),y],non_sequences=[y_score_given_x,x])
        cost = self.layer3.negative_log_likelihood(sy)
        ...
        self.validate_p = theano.function(
            ...
            [testnorm] + testgrads,
            givens={
                x: self.valid_set_x[self.index * self.train_batch_size: (self.index + 1) * self.train_batch_size]
            }
        )
        ...
        self.test_model = theano.function(
            ...
            givens={
                x: self.test_set_x[self.index * self.train_batch_size:(self.index + 1) * self.train_batch_size],
                y: self.test_set_y[self.index * self.train_batch_size: (self.index + 1) * self.train_batch_size]
            }
        )
        ...
        self.validate_model = theano.function(
            ...
            givens={
                x: self.valid_set_x[self.index * self.train_batch_size: (self.index + 1) * self.train_batch_size],
                y: self.valid_set_y[self.index * self.train_batch_size: (self.index + 1) * self.train_batch_size]
            }
        )
        ...
        self.train_model = theano.function(
            ...
            updates=updates,
            givens={
                x: self.train_set_x[self.index * self.train_batch_size:(self.index + 1) * self.train_batch_size],
                sy: self.train_set_sy[self.index * self.train_batch_size:(self.index + 1) * self.train_batch_size]
            }
        )
...
if __name__ == '__main__':
    ...
    theano.sandbox.cuda.use("gpu4")
    temperature = 10
    softlabel = './weight/normal_weights_RandInit_Temperature' + str(temperature) + '_softlabel.pkl'
    nn = LeNet(n_epochs=40,temperature=temperature,softlabel=softlabel)
    params = nn.train()
    f = open('./weight/normal_weights_RandInit_Distiliation_10.pkl', 'wb')
