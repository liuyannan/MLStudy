...
def resume_lenet5(old_parameter, thval = 1, nkerns=[20, 50, 500], batch_size=1, remove_edge_number=0):
...
if __name__ == '__main__':
    ...
    for m in model[4:]:
        ...
        flatten_params = []
        for i in range(8):
            normal_group = numpy.reshape(normal_params[i].get_value(), (-1))
            flatten_params += list(normal_group)

        flatten_params = map(lambda x: abs(x), flatten_params)
        flatten_params.sort()

        thval = flatten_params[remove_edge]

        for remove_edge in range(50000,300000,20000):
            gradsfunc, gradsfunc_l2, proby, scorefunc, scorefunc_l2, new_params = resume_lenet5(normal_params,remove_edge_number=remove_edge,thval=thval)
