...
def remove_weights(nn,ratio,policy):
    ...
    for i in range(len(nn.params)):
        ...
        mask_group = nn.masks[i].get_value()
        flatten_params += list(numpy.reshape(params_group * mask_group,(-1)))
...
def SW_CC_compression(folder,cf,ratio=0.1):
    '''
    ...
    while nn.connection_count() > 0:
        ...
        if epoch % 2 != 0:
            remove_weights(nn=nn,ratio=ratio,policy='SW')
        else:
            remove_weights(nn=nn, ratio=ratio, policy='CC')
...
if __name__ == '__main__':
    ...
    # folders =  [['NO_REGULAR','no_regular'], ['CONTRACT_LIKE','Contract_Likelihood'],['L2EN3','L2'], ['LOGINIT_CONTRACT_LIKE','Contract_Likelihood'],['LOGINIT_L2EN3','L2'],['LOGINIT_NO_REGULAR','no_regular']]

    folders = [['LOGINIT_CONTRACT_LIKE_L2', 'Contract_Likelihood_L2']]
    for folder in folders:
        compression(folder[0], cf=folder[1])
    ...
    # fixed_compression(mask_folder='L2EN3',target_floder='FC_CONTRACT_LIKE_L2MASK',target_cf='Contract_Likelihood')
#