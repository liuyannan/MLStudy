...
class FCSoftMaxLayer(object):
    ...
    def __init__(self, input, n_in, n_out, activation=T.tanh):
        ...
        # hyperplane-k
        self.p_y_given_x = T.nnet.logsoftmax(T.dot(input, self.W) + self.b)
...
def find_ad_sample_backtracking_step1_random_direction(gradsfunc, proby, score_func, image, target_class):
    transform_img = [numpy.copy(image)]
    conflist = []
    epoch = 0
    randomp = numpy.random.rand(784, 1)
    randomp *= 10e-30/(numpy.sum(randomp ** 2)) ** 0.5
    while epoch < 50000:
        random_direction = False
        probyvalue = numpy.exp(proby(transform_img, target_class))
        gradsvalue = gradsfunc(transform_img, [image], target_class)
        gradmag = (numpy.sum(gradsvalue ** 2)) ** 0.5
        if numpy.isnan(probyvalue):
            print("proby is nan, epoch %i"%epoch)
            probyvalue = 0.
        if epoch % 10 == 0:
        ...
        if numpy.isnan(numpy.sum(p)):
            nan_index = numpy.isnan(p)
            nan_count = numpy.sum(nan_index)
            p[nan_index] = (1./nan_count)**0.5 * 1e-30
        if not numpy.count_nonzero(p):
            p = numpy.copy(randomp)
            random_direction = True
        p *= 0.01 / gradmag
        t = p.T.dot(p) * 0
        step_size = 20.
        f_x_k = numpy.nan_to_num(numpy.exp(score_func(transform_img, [image], target_class)))
        ...
        if random_direction:
            while numpy.nan_to_num(numpy.exp(score_func([predict_img], [image], target_class))) <= f_x_k:
                # print("random_direction, epoch %i"%epoch)
                print("fake score is %e"%numpy.exp(score_func([predict_img], [image], target_class)))
                step_size += 20
                predict_img = transform_img[0] + step_size * p.reshape(784, )
                predict_img = numpy.maximum(lower_bound_img, numpy.minimum(predict_img, upper_bound_img))
        else:
            while numpy.exp(score_func([predict_img], [image], target_class)) < f_x_k + step_size * t:
                print(numpy.exp(score_func([predict_img], [image], target_class)))
                # print(gradsvalue)
                predict_img = transform_img[0] + step_size * p.reshape(784, )
                predict_img = numpy.maximum(lower_bound_img, numpy.minimum(predict_img, upper_bound_img))
                step_size *= 0.8
...
def find_ad_sample_backtracking_step1_softmax(gradsfunc, proby, score_func, image, target_class):
    transform_img = [numpy.copy(image)]
...
def dump_mnist_cifar_random(fname, gradsfunc, gradsfunc_l2, proby, scorefunc, scorefunc_l2):
    ...
    # f = open('./eval_efforts/Constraint_mnist_GDBack_'+fname+'.pkl', 'wb')
    ...
    #         confidence_data.append([[test_y[i], target_class], [ori_img, reimg], confidence_list])
    # f = open('./eval_efforts/Constraint_cifar_GDBack' + fname + '.pkl', 'wb')
    # pickle.dump(confidence_data, f, protocol=pickle.HIGHEST_PROTOCOL)
    # f.close()
    #
     # Random
     ...
     f = open('./eval_efforts/Constraint_random_GDBack' + fname + '.pkl', 'wb')
     pickle.dump(confidence_data, f, protocol=pickle.HIGHEST_PROTOCOL)
     f.close()


if __name__ == '__main__':
    theano.sandbox.cuda.use("gpu5")
    ...
    model = ["RandInit_Contract5_all_e0",
             ...
             "RandInit_L2R_en2",
             "RandInit"]
    model = [ "RandInit_Temperature10",
            "RandInit_Distiliation_10"]
    for m in model:
        f = open('./weight/normal_weights_' + m + '.pkl', 'rb')
        normal_params = pickle.load(f)
        f.close()
        gradsfunc, gradsfunc_l2, proby, scorefunc, scorefunc_l2 = resume_lenet5(normal_params)
        dump_mnist_cifar_random(m, gradsfunc, gradsfunc_l2, proby, scorefunc, scorefunc_l2)
