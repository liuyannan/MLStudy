...
class LeNet(object):
    def __init__(self, mu=0.5, learning_rate=0.1, n_epochs=40, dataset='mnist.pkl.gz', nkerns=[20, 50, 500],
                 batch_size=500, lam_contractive=10, lam_l2=0.001):
        ...
        self.layer1 = LeNetConvPoolLayer(self.rng, input=layer1_input,
                                         image_shape=(self.train_batch_size, nkerns[0], 12, 12), \
        ...
        self.layer3 = FCSoftMaxLayer(input=layer3_input, n_in=nkerns[2], n_out=10, rng=self.rng)
...
if __name__ == '__main__':
    ...
    for config in configuration:
        nn = LeNet(lam_contractive=1,nkerns=config)
        params,testscore = nn.train()
        score_list.append(testscore)
        name = [str(nnum) for nnum in config]
        f = open('./weight/normal_weights_RandInit_'+'_'.join(name)+'.pkl', 'wb')
        pickle.dump(params, f, protocol=pickle.HIGHEST_PROTOCOL)
        pickle.dump([config,testscore], f, protocol=pickle.HIGHEST_PROTOCOL)
        f.close()
    f = open('./weight/normal_weights_RandInit_L2R.score', 'w')
    for ind in range(len(configuration)):
        f.writelines(['_'.join(configuration[ind]) + ':' + str(score_list[ind]) + '\n'])
    f.close()
