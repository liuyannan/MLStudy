...
class LeNet(object):
    def __init__(self, mu=0.5, learning_rate=0.1, n_epochs=40, dataset='mnist.pkl.gz', nkerns=[20, 50],
                 batch_size=500, lam_contractive=1, lam_l2=0.01):
        """ Demonstrates lenet on MNIST dataset
        ...
        #                       ,sequences=T.arange(self.train_batch_size),non_sequences=[y_score_given_x,layer3_input])
        ...
        #                                                ind, :]
        #     , sequences=T.arange(self.train_batch_size), non_sequences=[layer3_input, layer2_input])
        #
        ...
        #                                                ind, :,:,:]
        #     , sequences=T.arange(self.train_batch_size), non_sequences=[layer2_input, layer1_input])
        #
        ...
        #                                                ind, :,:,:]
        #     , sequences=T.arange(self.train_batch_size), non_sequences=[layer1_input_flatten, layer0_input])
        #
        ...
        layer3_Jnorm = layer3_Jnorm1.flatten(2)
        Layer3_J_l2 = T.mean((T.sum((layer3_Jnorm**2),axis=1)**0.5))
        #cost = self.layer3.negative_log_likelihood(y) + lam_contractive * (Layer3_J_l2)  + lam_l2 * paramssum
        ...
        self.train_model = theano.function(
            ...
            givens={
                ...
                y: self.train_set_y[self.index * self.train_batch_size:(self.index + 1) * self.train_batch_size]
            }
        )
...
def find_fault_injection_points(normal_parameter, adver_parameter, adver_derivative, adver_dict, dataset='mnist.pkl.gz', nkerns=[20, 50], batch_size=500):
    """ Demonstrates lenet on MNIST dataset
...
if __name__ == '__main__':
    ...
    nn = LeNet(lam_contractive=1,n_epochs=6,lam_l2=0.01)
    params = nn.train()
    f = open('./weight/normal_weights_RandInit_Contract5_e0.pkl', 'wb')
