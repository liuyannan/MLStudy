...
class LeNet_DW(object):
    def __init__(self, mu=0.5, learning_rate=0.1, n_epochs=40, dataset='mnist.pkl.gz', nkerns=[20, 50],
                 batch_size=500, lam_l2=0.001, train_divisor=1, cf_type='L2',lam_contractive=1000,random_seed = 23455 ):
...
def dump_mnist(fname, gradsfunc, proby, folder):
    ...
    for i in range(200):
        ...
        _, ori_pred_class = proby([ori_img], target_class)
        if ori_pred_class != test_y[i]:
            continue
        for target_class in target_class_list:
            confidence_list, reimg, cur_class = find_ad_sample_backtracking_step1_logsoftmax(gradsfunc, proby, ori_img, target_class, test_y[i])
...
def eval_contractive_term(folder):
    '''
...
if __name__ == '__main__':
    ...
    for folder in folders:
        ...
        compression_API(folder[0], cf=folder[1], rm_policy= folder[2],resume=False, ratio=0.1)
        ...
        eval_adversarial_efforts(files, folder[0])
        eval_accuracy(folder[0])
        eval_contractive_term(folder[0])
