...
from CifarNet_mask_fork import ReLuConvLayer, MaxPoolingLayer, FCSoftMaxLayer, FCLayer, SoftMaxLayer, IdenConvLayer
from CifarNet_mask_fork import load_data, compression_API, eval_accuracy, eval_adversarial_efforts, eval_contractive_term
...
class NINNet(object):
    def __init__(self, initial_mu=0.9, initial_learning_rate=0.005, n_epochs=50, dataset='./cifar-10',
                 ...
                 batch_size=128, lam_l2=0.001, train_divisor=1, cf_type='L2', lam_contractive=100, weight_decay = 0.001):
        ...
        delta_L_to_x = T.grad(1000*self.layer9.negative_log_likelihood(y), x)
...
if __name__ == '__main__':
    ...
    folders = [
        #['NIN_LOGINIT_NO_REGULAR_100epochs', 'no_regular', 'SW', ''],
        ['NIN_CONTRACT_LIKE_100epochs', 'Contract_Likelihood', 'SW', '']
    ]
    ...
    for folder in []:
        ...
        # fixed_compression(mask_folder=folder[3], target_floder=folder[0], target_cf=folder[1])
            NIN = NINNet(cf_type='no_regular', initial_learning_rate=0.005)
            compression_API(folder[0], cf=folder[1], rm_policy=folder[2], resume=False, ratio=0.1, nn=NIN)
        ...
        print("this is"+ files[0])
        NIN = NINNet(batch_size = 1)
        eval_adversarial_efforts(files, folder[0],NIN)
        #NIN = NINNet()
        #eval_accuracy(folder[0],NIN)
        #eval_contractive_term(folder[0],NIN)

    nn = NINNet(cf_type='Contract_Likelihood', lam_contractive=0.5, initial_learning_rate=0.005)
    ...
    for i in range(len(grads)):
        ...
        print(grads[i])
    exit()
    #nn = NINNet(cf_type='no_regular', lam_contractive=0.5, initial_learning_rate=0.005)
    nn.train(100)
