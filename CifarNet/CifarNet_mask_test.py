...
def detect_nan(i, node, fn):
    for output in fn.outputs:
        if (not isinstance(output[0],numpy.random.RandomState) and numpy.isnan(output[0]).any()):
            print('***Nan Detected ***')
            theano.printing.debugprint(node,depth=2)
            print('Inputs:%s'%[iutput[0].shape for iutput in fn.inputs])
            print('Inputs has zero:%s'%[numpy.any(iutput[0]==0) for iutput in fn.inputs])
            print('Inputs has nan:%s'%[numpy.any(numpy.isnan(iutput[0])) for iutput in fn.inputs])
            print('Onputs:%s'%[output[0].shape  for output in fn.outputs])
            print('Ouputs has zero:%s'%[numpy.any(iutput[0]==0) for iutput in fn.outputs])
            print('Ouputs has nan:%s'%[numpy.any(numpy.isnan(iutput[0])) for iutput in fn.outputs])
            
            f = open('./log_no.pkl','a+b')
            pickle.dump([fn.inputs,fn.outputs], f, protocol=pickle.HIGHEST_PROTOCOL)
            f.close()
            break
...
class CifarNet(object):
    def __init__(self, initial_mu=0.9, initial_learning_rate=0.01, n_epochs=80, dataset='./cifar-10',
        ...
        self.rng = numpy.random.RandomState(38483)
    ...
    def recover_params(self):
        for i in range(len(self.params)):
            ...
            self.velocities[i].set_value(self.cur_velocities)
    ...
    def train(self, n_epochs):
        ...
        while (epoch < n_epochs) and (not done_looping):
            ...
            for minibatch_index in range(self.n_train_batches):
                ...
                #tmp = self.test_nan(minibatch_index)
                #for i in range(len(tmp)):
                    #print("%d has nan %s"%(i,numpy.isnan(tmp[i]))) 
                    #print("%d "%(i) )
                    #print(("%e, %e")%(numpy.amin(tmp[i]) ,numpy.amax(tmp[i])))
                    #print(tmp[i])
                #exit()
                self.log_cur_params()
                ...
                if patience <= iter:
                    ...
                    break
            if epoch%10 == 0:
                new_lr = self.learning_rate.get_value()*0.9
...
if __name__ == '__main__':
    ...
    nn = CifarNet(cf_type='Contract_Likelihood',lam_contractive=10,initial_learning_rate=0.001)
    nn.train(50)
